...
namespace BG
{
    ...
    const unsigned char GATT_HANDLE_TERMINAL_IN = 56;
    const unsigned char GATT_HANDLE_TERMINAL_OUT = 59;
...
//    const unsigned char  connectReq[] = {19,0x00,0x0f,0x06,0x03,0x5c,0xd3,0x60,0x80,0x07,0x00,0x00,0x3c,0x00,0x4c,0x00,0x64,0x00,0x00,0x00};
    ...
    const unsigned char gattHandles[] = 
    {
        ...
        GATT_HANDLE_TERMINAL_IN     , /* HANDLE_TERMINAL_IN,     */
        GATT_HANDLE_TERMINAL_OUT    , /* HANDLE_TERMINAL_OUT,    */
    };
...
public ref class Ble
{
    ...
    Ble( System::Windows::Forms::TextBox ^textBox )
    {
        ...
        _TerminalOutQ = gcnew Fifo( 50 );
        _TerminalInQ = gcnew Fifo( 50 );
    }
    ...
    int terminalWaiting( void )
    {
        return(_TerminalInQ->Count());
    }


    void readTerminal( array<char>^ buffer, int length )
    {
        for (int i = 0; i < length; ++i)
        {
            buffer[i] = (char)_TerminalInQ->Dequeue();
        }
    }


    void writeTerminal( array<char>^ buffer, int length )
    {
        int i;
        
        for (i = 0; i < length; ++i)
        {
            _TerminalOutQ->Enqueue( (char)buffer[i] );
        }
        

        issueCommand( BG::BLE_WRITE, BG::HANDLE_TERMINAL_IN );
    }
    ...
    {
        _test = true;
        issueCommand( BG::BLE_GET_HANDLES, 0 );
    }
                    ...
                    switch (_lastCommand)
                    {
                        ...
                        case BG::BLE_WRITE:
                            ...
                            switch (handle)
                            {
                                ...
                                case BG::HANDLE_TERMINAL_IN:
                                    _Outgoing[1] = (unsigned char)24;
                                    _Outgoing[7] = (unsigned char)20;
                                   
                                    /* Output the next 20 terminal bytes  - pad with 0s if necessary */
                                    for (int i = 0; i < 20; ++i)
                                    {
                                        if (_TerminalOutQ->Count() != 0)
                                        {
                                            _Outgoing->Add( (unsigned char)_TerminalOutQ->Dequeue() );
                                        }
                                        else
                                        {
                                            _Outgoing->Add( (unsigned char)0 );
 ...
 */
    ...
    int gattGetTrackingOnOff( void )
    {
        return(_trackingOnOff);
    }


    void gattSetTrackingOnOff( int setting )
    {
        _trackingOnOff = setting;
    }


    int gattGetTrackingPeriod( void )
    {
        return(_trackingPeriod);
    }


    void gattSetTrackingPeriod( int setting )
    {
        _trackingPeriod = setting;
    }
                    ...
                    case BG::BLE_EVT_NEW_VALUE:
                        if (isTest() == false)
                        {
                            switch ((unsigned char)_Incoming[5])
                            {
                                ...
                                case BG::GATT_HANDLE_TERMINAL_OUT:
                                    /* Incoming terminal, so add it to any previous */
                                    log( "TerminalIn" );
                                    for (int i = 0; i < 20; ++i)
                                    {
                                        _TerminalInQ->Enqueue((unsigned char)_Incoming[i + 9]);
                                    }
                                    break;
